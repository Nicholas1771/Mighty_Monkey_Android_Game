...
public class GoogleApiAvailability extends zze {
    ...
    private final void zza(Context context, int i, String str, PendingIntent pendingIntent) {
        ...
        if (i == 18) {
            ...
            if (zzj.zzaH(context)) {
                zzbo.zzae(zzq.zzsd());
                build = new Notification.Builder(context).setSmallIcon(context.getApplicationInfo().icon).setPriority(2).setAutoCancel(true).setContentTitle(zzh).setStyle(new Notification.BigTextStyle().bigText(zzj)).addAction(C0481R.C0482drawable.common_full_open_on_phone, resources.getString(C0481R.string.common_open_on_phone), pendingIntent).build();
            } else {
                build = new NotificationCompat.Builder(context).setSmallIcon(17301642).setTicker(resources.getString(C0481R.string.common_google_play_services_notification_ticker)).setWhen(System.currentTimeMillis()).setAutoCancel(true).setContentIntent(pendingIntent).setContentTitle(zzh).setContentText(zzj).setLocalOnly(true).setStyle(new NotificationCompat.BigTextStyle().bigText(zzj)).build();
            }
            switch (i) {
                case 1:
                case 2:
    ...
    public final boolean zza(Activity activity, @NonNull zzbdt zzbdt, int i, int i2, DialogInterface.OnCancelListener onCancelListener) {
        Dialog zza2 = zza((Context) activity, i, zzt.zza(zzbdt, zze.zza(activity, i, "d"), 2), onCancelListener);
        if (zza2 == null) {
            return false;
        }
