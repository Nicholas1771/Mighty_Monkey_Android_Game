...
public final class zzbbp implements zzbdp {
    ...
    public final ConnectionResult zzpN() {
        ...
        for (zzbbo next : this.zzaCB.values()) {
            Api zzpg = next.zzpg();
            ConnectionResult connectionResult4 = this.zzaCL.get(next.zzph());
            if (!connectionResult4.isSuccess() && (!this.zzaCD.get(zzpg).booleanValue() || connectionResult4.hasResolution() || this.zzaCF.isUserResolvableError(connectionResult4.getErrorCode()))) {
                if (connectionResult4.getErrorCode() != 4 || !this.zzaCH) {
